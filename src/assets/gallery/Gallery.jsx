...
// Default OFF so captions never show on the page.
// You can turn them back on by setting VITE_GALLERY_SHOW_CAPTIONS=true (optional).
const SHOW_CAPTIONS = (import.meta.env.VITE_GALLERY_SHOW_CAPTIONS || 'false') === 'true';

export default function Gallery() {
  const [activeKey, setActiveKey] = useState(
    () => categories.find(c => c.data.length > 0)?.key || categories[0].key
  );
  ...
  const prev = (e) => {
    e?.stopPropagation?.();
    setLightbox(s => ({ open: true, idx: (s.idx - 1 + active.data.length) % active.data.length }));
  };
  const next = (e) => {
    e?.stopPropagation?.();
    setLightbox(s => ({ open: true, idx: (s.idx + 1) % active.data.length }));
  };
    ...
    <section id="gallery" className="py-16 bg-onyx" aria-labelledby="gallery-title">
        ...
        <div className="mt-8 columns-1 sm:columns-2 lg:columns-3 gap-4 [column-fill:_balance]">
          {active.data.length === 0 && (
            <p className="text-gray-400 text-center">
              No images yet. Add files to <code className="bg-carbon/70 px-2 py-1 rounded">src/assets/gallery/{active.key}</code>
            </p>
          )}
          {active.data.map((src, idx) => (
            <figure
              key={src + idx}
              className="mb-4 break-inside-avoid overflow-hidden rounded-2xl shadow-soft gradient-ring cursor-zoom-in group"
              onClick={() => open(idx)}
            >
              <img
                src={src}
              ...
              />
              {SHOW_CAPTIONS ? (
                <figcaption className="text-left text-xs text-gray-400 p-2">Euphoria gallery image</figcaption>
              ) : (
                // Keep a non-visible caption for screen readers (no filename shown visually)
      ...
      {lightbox.open && active.data.length > 0 && (
        ...
        >
          ...
          <img
            src={active.data[lightbox.idx]}
            alt={altFrom(active.data[lightbox.idx])}
...
// Generic alt so filenames never surface
function altFrom(_url) {
  return 'Euphoria gallery image';
}
